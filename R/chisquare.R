...
edu_chisq_independence <- function(data, row, column, counts = NULL) {
    ...
    apa <- sprintf(
        "A chi-square test of independence found %s between %s and %s, chi-square(%s, N = %s) = %s, p %s, Cramer's V = %s.",
...
edu_chisq_gof <- function(data, variable, counts = NULL, expected = NULL) {
    ...
    apa <- sprintf(
        "A chi-square goodness-of-fit test indicated that frequencies for %s %s %s, chi-square(%s, N = %s) = %s, p %s, Cohen's w = %s.",
