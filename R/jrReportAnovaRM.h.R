...
jrReportAnovaRMOptions <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "jrReportAnovaRMOptions",
    inherit = jmvcore::Options,
    public = list(
        initialize = function( ...) {
            super$initialize(
                package="jReport",
                name="jrReportAnovaRM",
                requiresData=TRUE,
                ...)
        }),
    active = list(),
...
jrReportAnovaRMResults <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "jrReportAnovaRMResults",
    ...
    public = list(
        initialize = function(options, data=NULL, datasetId="", analysisId="", revision=0) {
            super$initialize(
                ...
                version = c(1,0,0),
                options = options,
                results = jrReportAnovaRMResults$new(options=options),
                data = data,
                datasetId = datasetId,
...
#'
#' @export
jrReportAnovaRM <- function(data) {
    if ( ! requireNamespace("jmvcore", quietly=TRUE))
        stop("jrReportAnovaRM requires jmvcore to be installed (restart may be required)")
    if (missing(data))
        data <- jmvcore::marshalData(parent.frame())
    options <- jrReportAnovaRMOptions$new()
    analysis <- jrReportAnovaRMClass$new(options = options, data = data)
    analysis$run()
    analysis$results
}
