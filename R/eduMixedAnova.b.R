#' @importFrom jmvcore .
eduMixedAnovaClass <- if (requireNamespace("jmvcore", quietly = TRUE)) R6Class(
    "eduMixedAnovaClass",
    inherit = eduMixedAnovaBase,
    private = list(
        .run = function() {
            if (length(self$options$measures) < 2L || is.null(self$options$group))
                return()
